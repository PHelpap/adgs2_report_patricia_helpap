...
1) The model could be optimized by using more than 1 station as input data for the model i.e. improve calibration parameters. This would increase the number of observations that are used to calculate the optimal paramters used in the model and should therefore make predictions more robust.
...
Downloading greenness time series of the original harvard station (+ two further sites that are closest to harvard to enhance *n* (hence hopefully improving the optimalisation parameters) and calculating phenology (phenophases).
...
optim_par_ha = GenSA::GenSA( #comparison with previous model using harvard alone
  par = par,
  fn = rmse_gdd,
  lower = lower,
  upper = upper,
...
plot_allsites <- ggplot(validation) + #trying to combine both plots into one??
  ...
  annotate("text", x = 121, y = 138,
           label = paste("optimized model"), color = "black", size = 4) +
  theme_bw()  +
...
plot_harvard <- ggplot(validation_ha) + #for comparing with harvard alone
  ...
  annotate("text", x = 121, y = 139,
           label = paste("Original model"), color = "red3", size = 4) +
  theme_bw()  +
  theme(
...
```
It becomes apparent quite quickly that including more sites into the optimal parameter function of the  model has actually made the predictions worse here compared to the observed DOY since the black line deviates even further from the ideal diagonal than the red line.

--- 

DAYMET data can then be used to scale the model spatially:
```{r spatial scaling}
# Download daily data
...
for statistical comparison (averaged spatially):
...
```{r stats plots, echo = FALSE, fig.cap = "Fig. 3: Statistical evaluation of observed MODIS DOY vs model prediction DOY averaged spatially for years 2008-2022. "}
...
I redownload MODIS data for years 2009, 2010, and 2011 but with 50km spatial scale instead of 20km and utilise the DAYMET data of the same years to plot the model predictions. For direct spatial comparison, I crop the DAYMET data to the same extend as the MODIS data.
```{r spatial comparison, echo=FALSE}
...
```{r spatial plots, fig.cap= "Fig. 4: DOY for the years 2009, 2010 and 2011 of observed MODIS data (top panels) and predicted DAYMET values (bottom panels)"}
#MODIS
...
- The Mann-Whitney-U-test shows that the two datasets are significantly different, i.e. we have to reject the null hypothesis that the observed and the predicted data from the optimized model are a close match. The resulting slope and intercept of the linear model show that the predicted values are significantly higher than the observed MODIS data for this region (Fig. 3). Correlation of both datasets is not as high as expected with 0.725; ideally, we would want a value much closer to 1. The R^2 value of 0.53 means that the predicted values can only explain 53% of the observed values.
